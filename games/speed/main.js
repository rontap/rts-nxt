...
Players = [];
Players.push( new Player("Zoltan" , 1) );
Players.push( new Player("Albert" , 0) );
...
function keyBoardControl(e) {
  ...
  if ((e.code == "ControlLeft") ||(e.key =="t")) {
    controlActive[0] = 3; console.log("CL");
    $$('holder[user]')[0].classList.add('move');

  }
  if (e.code == "ControlRight" ||(e.key =="u")) {
    controlActive[1] = 3; console.log("CR");
    $$('holder[user]')[1].classList.add('move');

  }
  if (e.code == "Tab") {
    if ((controlActive[0] == 2) && (controlActive[1] == 2)) {
      splash(0,1);
      controlActive = [ 0 , 0 ];
  ...
  switch (e.key) {
    case "a": u=0; k=0; controlActive[0]--; break;
    ...
    case "é": u=1; k=4; controlActive[1]--; break;
...
//0.9.4  bugfixes, speed and UI visual improvements <2017/11/28>
