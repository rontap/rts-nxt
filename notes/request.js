...
function openNote() {//or create
        ...
        xmlhttp.onreadystatechange = function() {
                ...
                if ( (this.status==200) && (this.readyState==3) ) {
                       ...
                       catch(e) { errorMsgHolder.innerHTML='Error: Corrupted File, JSON error.';  nxt.notify('#not_err',4000); }

                       console.log(this.responseText);
                       if (currentNote.error!=undefined) {
                               errorMsgHolder.innerHTML = currentNote.error;
                               nxt.notify('#not_err',4000);

                               if (currentNote.error=="Authentication Error! This note is private.") {
                                 setTimeout(() => $$('.omenu')[4].click(), 200);
